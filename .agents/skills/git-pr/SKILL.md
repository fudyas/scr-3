...
1. **Resolve the boundary** — the lower bound of the range. Run this block; it uses the requested base when given (whole branch since that base), else auto-detects the last merge, else the creation point:
   ```bash
   arg="<requested-base-or-empty>" # substitute the base supplied with $git-pr
