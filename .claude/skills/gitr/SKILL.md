---
name: gitr
description: Use this skill to review all pending git changes (staged and unstaged), group them into functionally related commits, present the proposed commit sequence to the user, and — only after explicit consent — commit everything using that sequence. Never splits files across commits (no `git add -p`); each file goes whole into exactly one commit. Do NOT auto-invoke; only run when explicitly called with /gitr.
allowed-tools: Bash(git --no-pager status:*), Bash(git --no-pager diff:*), Bash(git --no-pager log:*), Bash(git reset:*), Bash(git add:*), Bash(git commit:*)
argument-hint: "[optional grouping or focus hint]"
---
...
`$ARGUMENTS` — optional. A hint about how to group or what to emphasize (e.g. "keep docs separate", "one commit for the indexer refactor"). Honor it when forming groups and writing messages. If empty, derive grouping from the changes alone.
